...
#     dataPath = '/home/zmh/data/waymo/processed/training/023'
...
for idx, ego_pose_file in enumerate(ego_pose_name, start=1):
    ...
    ego_pose_matrix_str = np.loadtxt(ego_pose_file, dtype=str)
    # 将字符串转换为 Decimal 数值类型
    ego_pose_matrix = np.array([[Decimal(value) for value in row] for row in ego_pose_matrix_str], dtype=object).astype(float)

    # 计算ego_pose的逆矩阵
    ego_pose_inv = np.linalg.inv(ego_pose_matrix)
    _cam_to_ego = ego_pose_inv @ cam_to_ego
